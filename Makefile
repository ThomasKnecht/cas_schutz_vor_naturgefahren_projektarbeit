...
sync_mesh_base_data:
	rsync -rlv --checksum "./meshes/base_data/" "$(username)@$(server):$(project_dir)/meshes/base_data/" 
...
sync_output_basemesh:
	rsync -rlv --checksum "$(username)@$(server):$(project_dir)/meshes/$(mesh_name)/" "./meshes/$(mesh_name)/"

sync_uv_lock:
	rm ./src/python/uv.lock && \
    rsync -rlv --checksum "$(username)@$(server):$(project_dir)/meshes/uv.lock" "./python/uv.lock"


sync_model:
	rsync -rlv --checksum "./models/$(model_name)/" "$(username)@$(server):$(project_dir)/models/$(model_name)/" && \
    rsync -rlv --checksum "./bash/model_workflow.sh" "$(username)@$(server):$(project_dir)/models/model_workflow.sh"
